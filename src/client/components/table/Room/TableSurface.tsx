...
// Столешницу мы видим сильно уменьшенной и сильно сжатой по вертикали (см.
// tableTopSquash): картинка в тысячу точек ложится в полосу высотой в две-три
// сотни. Без мипмапов такое уменьшение осыпается в рябь — заклёпки, доски и
// прорезанные стрелки распадаются в пыль, и стол выглядит «пиксельным», хотя
// сглаживание включено: оно работает на краях фигур, а не внутри заливки.
//
// Мипмапы pixi строит только для картинок со стороной в степень двойки (см.
// MIPMAP_MODES.POW2, а на WebGL 1 это и вовсе единственный вариант) — поэтому
// table.jpg лежит ровно 1024×1024, и менять её размер нельзя.
//
// Одних мипмапов мало: по горизонтали картинка сжата вдвое с небольшим, а по
// вертикали — впятеро, и общий уровень выбирается по худшей оси, замыливая
// стол поперёк. Ровно это лечит анизотропия, её и просим; по умолчанию у pixi
// она нулевая.
topTexture.baseTexture.anisotropicLevel = 16;
